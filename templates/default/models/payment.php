    ...
    <div id="content" class="pt30 pb30">
        <div class="container">
            ...
            <div class="row mb30" id="booking-breadcrumb">
                ...
                <div class="col-sm-2">
                    <a href="<?php echo DOCBASE.$pms_sys_pages['summary']['alias']; ?>">
                        <div class="breadcrumb-item done">
            ...
            if($payment_type == 'paypal'){ ?>
                <div class="text-center">
                    <?php echo $pms_texts['PAYMENT_PAYPAL_NOTICE']; ?><br>
                    <img src="<?php echo pms_getFromTemplate('images/paypal-cards.png'); ?>" alt="PayPal" class="img-responsive mt10 mb30">
                    <form action="https://www.<?php if(PMS_PAYMENT_TEST_MODE == 1) echo 'sandbox.'; ?>paypal.com/cgi-bin/webscr" method="post">
                        <input type='hidden' value="<?php echo number_format(str_replace(',', '.', $payed_amount), 2, '.', ''); ?>" name="amount">
                        <input name="currency_code" type="hidden" value="<?php echo PMS_DEFAULT_CURRENCY_CODE; ?>">
                        <input name="shipping" type="hidden" value="0.00">
                        <input name="tax" type="hidden" value="0.00">
                        <input name="return" type="hidden" value="<?php echo pms_getUrl(true).DOCBASE.$pms_sys_pages['booking']['alias'].'?action=confirm'; ?>">
                        <input name="cancel_return" type="hidden" value="<?php echo pms_getUrl(true).DOCBASE.$pms_sys_pages['booking']['alias'].'?action=cancel'; ?>">
                        <input name="notify_url" type="hidden" value="<?php echo pms_getUrl(true).DOCBASE.'includes/payments/paypal_notify.php'; ?>">
                        <input name="cmd" type="hidden" value="_xclick">
                        <input name="business" type="hidden" value="<?php echo PMS_PAYPAL_EMAIL; ?>">
                        <input name="item_name" type="hidden" value="<?php echo addslashes(gmstrftime(PMS_DATE_FORMAT, $_SESSION['tmp_book']['from_date']).' > '.gmstrftime(PMS_DATE_FORMAT, $_SESSION['tmp_book']['to_date']).' - '.$_SESSION['tmp_book']['nights'].' '.$pms_texts['NIGHTS'].' - '.($_SESSION['tmp_book']['adults']+$_SESSION['tmp_book']['children']).' '.$pms_texts['PERSONS']); ?>">
                        <input name="no_note" type="hidden" value="1">
                        <input name="lc" type="hidden" value="<?php echo strtoupper(PMS_LANG_TAG); ?>">
                        <input name="bn" type="hidden" value="PP-BuyNowBF">
                        <input name="custom" type="hidden" value="<?php echo $_SESSION['tmp_book']['id']; ?>">
                        
                        <button type="submit" name="submit" class="btn btn-primary btn-lg"><i class="fab fa-fw fa-paypal"></i> <?php echo $pms_texts['PAY']; ?></button>
                    </form>
                </div>
                <?php
            }elseif($payment_type == '2checkout'){ ?>
                <div class="text-center">
                    <?php echo $pms_texts['PAYMENT_2CHECKOUT_NOTICE']; ?><br>
                    <img src="<?php echo pms_getFromTemplate('images/2checkout-cards.png'); ?>" alt="2Checkout.com" class="img-responsive mt10 mb30">
                    <form action="https://<?php if(PMS_PAYMENT_TEST_MODE == 1) echo 'sandbox'; else echo 'www'; ?>.2checkout.com/checkout/purchase" method="post">
                        <input type="hidden" name="sid" value="<?php echo PMS_VENDOR_ID; ?>">
                        <input type="hidden" name="currency_code" value="<?php echo PMS_DEFAULT_CURRENCY_CODE; ?>">
                        <input type="hidden" name="lang" value="<?php echo PMS_LANG_TAG; ?>">
                        <input type="hidden" name="mode" value="2CO">
                        <input type="hidden" name="merchant_order_id" value="<?php echo $_SESSION['tmp_book']['id']; ?>">
                        <input type="hidden" name="li_0_type" value="product">
                        <input type="hidden" name="li_0_name" value="<?php echo addslashes(gmstrftime(PMS_DATE_FORMAT, $_SESSION['tmp_book']['from_date']).' > '.gmstrftime(PMS_DATE_FORMAT, $_SESSION['tmp_book']['to_date']).' - '.$_SESSION['tmp_book']['nights'].' '.$pms_texts['NIGHTS'].' - '.($_SESSION['tmp_book']['adults']+$_SESSION['tmp_book']['children']).' '.$pms_texts['PERSONS']); ?>">
                        ...
                        <input type="hidden" name="x_receipt_link_url" value="<?php echo pms_getUrl(true).DOCBASE.'includes/payments/2checkout_notify.php'; ?>">
                        
                        <button type="submit" name="submit" class="btn btn-primary btn-lg"><i class="fas fa-fw fa-credit-card"></i> <?php echo $pms_texts['PAY']; ?></button>
                    </form>
                </div>
                <?php
            }elseif($payment_type == 'braintree'){ ?>
                <div class="text-center">
                    <?php echo $pms_texts['PAYMENT_BRAINTREE_NOTICE']; ?><br>
                    <img src="<?php echo pms_getFromTemplate('images/braintree-cards.jpg'); ?>" alt="Braintree" class="img-responsive mt10 mb30">
                    <form action="<?php echo DOCBASE; ?>includes/payments/braintree/checkout.php" method="post">
						...
						<button type="submit" class="btn btn-primary btn-lg" id="braintree_btn" style="display: none;"><i class="fas fa-fw fa-credit-card"></i> <?php echo $pms_texts['PAY']; ?></button>
					</form>
                </div>
                <?php
            }elseif($payment_type == 'razorpay'){ ?>
                <div class="text-center">
                    <?php echo $pms_texts['PAYMENT_RAZORPAY_NOTICE']; ?><br>
                    <img src="<?php echo pms_getFromTemplate('images/razorpay-cards.jpg'); ?>" alt="Razorpay" class="img-responsive mt10 mb30">
                    <form action="<?php echo DOCBASE; ?>includes/payments/razorpay_notify.php" method="post">
						<script
							src="https://checkout.razorpay.com/v1/checkout.js"
							data-key="<?php echo PMS_RAZORPAY_KEY_ID; ?>"
                            data-order_id="<?php echo $_SESSION['tmp_book']['id']; ?>"
							data-amount="<?php echo round($payed_amount*100, 0); ?>"
							data-currency="INR"
							data-buttontext="<?php echo $pms_texts['PAY']; ?>"
							data-name="<?php echo PMS_SITE_TITLE; ?>"
							data-description=""
							data-image="<?php echo pms_getUrl(true).pms_getFromTemplate('images/logo.png'); ?>"
                ...
                <div class="text-center lead pt20 pb20">
                    ...
                    <form method="post" action="<?php echo DOCBASE.$pms_sys_pages['payment']['alias']; ?>">
                        ...
                        if(!isset($_POST['payment_type'])){
                            $payments = array_map('trim', explode(',', PMS_PAYMENT_TYPE));
                            if(count($payments) > 1){ ?>
                                <div class="mb10">
                                    <?php echo $pms_texts['CHOOSE_PAYMENT']; ?>
                                </div>
                                <?php
                                foreach($payments as $payment){ ?>
                                    <button type="submit" name="payment_type" class="btn btn-default" value="<?php echo $payment; ?>">
                                        ...
                                        switch($payment){
                                            ...
                                            break;
                                            case 'check': ?>
                                                <i class="fas fa-fw fa-envelope"></i><br><?php echo $pms_texts['PAYMENT_CHECK']; ?>
                                                <?php
                                            break;
                                            case 'arrival': ?>
                                                <i class="fas fa-fw fa-building"></i><br><?php echo $pms_texts['PAYMENT_ARRIVAL']; ?>
                                                <?php
                                            break;
                                        } ?>
                                    </button>
                    ...
                    <?php
                    if($payment_type == 'check') echo str_replace('{amount}', '<b>'.pms_formatPrice($payed_amount, PMS_DEFAULT_CURRENCY_SIGN).' '.$pms_texts['INCL_VAT'].'</b>', $pms_texts['PAYMENT_CHECK_NOTICE']);
                    
                    if($payment_type == 'arrival') echo str_replace('{amount}', '<b>'.pms_formatPrice($payed_amount*PMS_CURRENCY_RATE).' '.$pms_texts['INCL_VAT'].'</b>', $pms_texts['PAYMENT_ARRIVAL_NOTICE']); ?>
                </div>
                    
                <div class="clearfix"></div>
                <a class="btn btn-default btn-lg pull-left" href="<?php echo DOCBASE.$pms_sys_pages['summary']['alias']; ?>"><i class="fas fa-fw fa-angle-left"></i> <?php echo $pms_texts['PREVIOUS_STEP']; ?></a>
