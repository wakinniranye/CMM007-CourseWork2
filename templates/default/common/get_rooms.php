...
if(isset($pms_db) && $pms_db !== false){
    ...
    $result_room = $pms_db->query('SELECT * FROM pm_room WHERE lang = '.PMS_LANG_ID.' AND checked = 1 ORDER BY `rank` LIMIT '.($lz_offset-1)*$lz_limit.', '.$lz_limit);

    $room_id = 0;

    $result_room_file = $pms_db->prepare('SELECT * FROM pm_room_file WHERE id_item = :room_id AND checked = 1 AND lang = '.PMS_LANG_ID.' AND type = \'image\' AND file != \'\' ORDER BY `rank` LIMIT 1');
    ...
    foreach($result_room as $i => $row){
        ...
        $room_alias = DOCBASE.$page_alias.'/'.pms_text_format($room_alias);
