...
if(isset($pms_db) && $pms_db !== false){
    ...
    $result_article_all_files = $pms_db->prepare('SELECT * FROM pm_article_file WHERE id_item = :article_id AND checked = 1 AND lang = '.PMS_DEFAULT_LANG.' AND type = \'image\' AND file != \'\' ORDER BY `rank`');
    $result_article_all_files->bindParam(':article_id', $pms_article_id);
    ...
    foreach($result_article as $i => $row){
        ...
        <article class="col-sm-4 isotopeItem'.$article_tags.'" itemscope itemtype="http://schema.org/Article">
            <div class="isotopeInner">
                <a itemprop="url" class="popup-modal" href="#article-popup-'.$pms_article_id.'">';
                    ...
                    <div class="isotopeContent">
                        ...
                        if($article_text != '') $html .= '<p>'.pms_strtrunc(pms_rip_tags($article_text),170).'</p>';
                        $html .= '
                        <div class="more-btn">
                            <span class="btn btn-primary">'.$pms_texts['READMORE'].'</span>
                        </div>
                    </div>
                </a>
            </div>
        </article>
        <div id="article-popup-'.$pms_article_id.'" class="white-popup-block mfp-hide">
            <div class="fluid-container">
                <div class="row">
                    <div class="col-xs-12 mb20">
                        <div class="owl-carousel" data-items="1" data-autoplay="true" data-dots="true" data-nav="false" data-rtl="'.((PMS_RTL_DIR) ? 'true' : 'false').'">';
                    ...
                    <div class="col-xs-12">
                        ...
                        $html .= '<div class="mb10 mt10 labels" dir="ltr">
                            <span class="label label-default"><i class="fa fa-thumb-tack"></i> '.((!PMS_RTL_DIR) ? strftime(PMS_DATE_FORMAT, $article_add_date) : strftime('%F', $article_add_date)).'</span>
                            <span class="label label-default"><i class="fa fa-comment"></i>  '.$nb_comments.' '.mb_strtolower($pms_texts['COMMENTS'], 'UTF-8').'</span>';
                            
                            $result_tag = $pms_db->query('SELECT * FROM pm_tag WHERE id IN ('.$article_tags.') AND checked = 1 AND lang = '.PMS_LANG_ID.' ORDER BY `rank`');
                            if($result_tag !== false){
                        ...
                        $short_text = pms_strtrunc(pms_rip_tags($article_text), 100);
                        $site_url = pms_getUrl();
