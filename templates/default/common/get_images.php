...
if(isset($_POST['ajax']) && $_POST['ajax'] == 1){
    ...
    && isset($_POST['article_id']) && is_numeric($_POST['article_id'])){
        $pms_article_id = $_POST['article_id'];
...
if(isset($pms_db) && $pms_db !== false){
    ...
    if(isset($pms_article_id)){

        $result_article_file = $pms_db->query("SELECT * FROM pm_article_file WHERE id_item = ".$pms_article_id." AND checked = 1 AND lang = ".PMS_DEFAULT_LANG." AND type = 'image' AND file != '' ORDER BY `rank` LIMIT ".($lz_offset-1)*$lz_limit.", ".$lz_limit);
