        ...
        if(!empty($itemValues)){
            ...
            if($valueList->length > 0){
                foreach($valueList as $value)
                    $caseValues[htmlentities($value->getAttribute('case'), ENT_QUOTES, 'UTF-8')] = pms_get_translation($value->nodeValue);
            }
        }
 ...
 *
 */
function getColsValues($pms_db, $row, $i, $cols)
{
    foreach($cols as $col){
        $table = $col->getTable();
        $colname = $col->getName();
        ...
        if($table != ''){
            ...
            if($pms_db !== false && pms_db_table_exists($pms_db, $table)){
                
                if(preg_match('/.*(int).*/i', pms_db_column_type($pms_db, $table, $col->getFieldRef())) === false || !empty($value)){
                    
                    if($value == 0) $value = '-';
                    else{
                        $req_table = 'SELECT * FROM '.$table.' WHERE `'.$col->getFieldRef().'` IN('.$value.')';
                        if(pms_db_column_exists($pms_db, $table, 'lang')) $req_table .= ' AND lang = '.PMS_DEFAULT_LANG;
                        $res_table = $pms_db->query($req_table);
                        if($res_table !== false){
                            $value = '';
            ...
            if($lgt1 > 1){
                ...
                if(!is_null($value)){
                    switch($col->getType()){
                        case 'date' :
                            $value = gmstrftime(PMS_DATE_FORMAT, $value);
                        break;
                        case 'datetime' :
                            $value = gmstrftime(PMS_DATE_FORMAT.' '.PMS_TIME_FORMAT, $value);
                        break;
                        case 'price' :
                            $value = pms_formatPrice($value, PMS_DEFAULT_CURRENCY_SIGN);
                        break;
                        case 'case' :
        ...
        if($optionTable != '' && $fieldLabel != '' && $fieldValue != ''){
            if($optionList->length > 0){
                foreach($optionList as $option)
                    $options[htmlentities($option->getAttribute('value'), ENT_QUOTES, 'UTF-8')] = pms_get_translation($option->nodeValue);
            }
