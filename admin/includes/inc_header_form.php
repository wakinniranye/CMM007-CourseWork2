...
<script src="<?php echo DOCBASE.PMS_ADMIN_FOLDER; ?>/js/plugins/ckeditor5/build/ckeditor.js"></script>
<script src="<?php echo DOCBASE.PMS_ADMIN_FOLDER; ?>/js/plugins/ckeditor5/build/init.js"></script>
...
<script>
    ...
    $(function(){
        ...
        $('.typeahead').each(function(){
            var input = $(this);
            var target = input.attr('rel');
            pms_init_typeahead(input);
        });
        
        $('select[name="multiple_actions_file"]').on('change', function(){
            if(($(this).val() == 'delete_multi_file' && confirm('<?php echo $pms_texts['DELETE_CONFIRM1']." ".$pms_texts['LOOSE_DATAS']; ?>'))
            || ($(this).val() != 'delete_multi_file' && confirm('<?php echo $pms_texts['ACTION_CONFIRM']." ".$pms_texts['LOOSE_DATAS']; ?>')))
                $('#form').attr('action','index.php?view=form&csrf_token=<?php echo $csrf_token; ?>&action='+$(this).val()).trigger('submit');
        });
        ...
        $('.actions-file a').on('click', function(){
            if(!confirm('<?php echo $pms_texts['ACTION_CONFIRM']." ".$pms_texts['LOOSE_DATAS']; ?>')) return false;
        });
...
if(NB_FILES > 0 && (in_array("add", $permissions) || in_array("edit", $permissions) || in_array("all", $permissions))){ ?>
    <script src="<?php echo DOCBASE.PMS_ADMIN_FOLDER; ?>/includes/uploadifive/jquery.uploadifive.js"></script>
    <link href="<?php echo DOCBASE.PMS_ADMIN_FOLDER; ?>/includes/uploadifive/uploadifive.css" rel="stylesheet">
    <script src="<?php echo DOCBASE.PMS_ADMIN_FOLDER; ?>/js/toolMan.js"></script>
    <script>
        ...
        $(function() {
            ...
            $('.files-list').each(function(){
                ...
                function saveOrder(item) {
                    ...
                    group.register('dragend', function(){
                        ...
                        $.ajax({
                            type: "POST",
                            url: '<?php echo DOCBASE.PMS_ADMIN_FOLDER; ?>/includes/order_medias.php?list='+list+'&table=<?php echo MODULE."_file"; ?>&id_item=<?php echo $id; ?>&prefix=file'
                        });
                    })
                }
            });
            ...
            if(empty($_SESSION['msg_error'])){ ?>
                $.ajax({
                    type: 'POST',
                    url: '<?php echo DOCBASE.PMS_ADMIN_FOLDER; ?>/includes/clear_tmp.php',
            ...
            $('.file_upload').each(function(){
                ...
                $('#'+id).uploadifive({
                    'formData'         : {
                        ...
                        'dir' : '<?php echo MODULE; ?>',
                        'root_bo' : '<?php echo DOCBASE.PMS_ADMIN_FOLDER; ?>/',
                        'exts' : '<?php echo serialize(array_keys($pms_allowable_file_exts)); ?>',
                        'lang' : lang
                    },
                    'buttonText'     : '<i class="fa fa-folder-open"></i> <?php echo $pms_texts['CHOOSE_FILE']; ?>',
                    'fileTypeDesc'     : 'Files',
                    'fileTypeExts'     : '<?php foreach(array_keys($pms_allowable_file_exts) as $file_ext) echo "*.".$file_ext.";*.".mb_strtoupper($file_ext, "UTF-8").";"; ?>',
                    'multi'            : (max_file > 1),
                    'queueSizeLimit': max_file,
                    'uploadLimit'     : max_file,
                    'queueID'        : 'file_upload_'+lang+'-queue',
                    'uploadScript'     : '<?php echo DOCBASE.PMS_ADMIN_FOLDER; ?>/includes/uploadifive/uploader/uploadifive.php',
                    'onUploadComplete' : function(file, data, response){
                        ...
                        if((data[2] == 0 && data[3] == 0) || ext == 'swf'){
                            ...
                            switch(ext){
                                <?php
                                foreach($pms_allowable_file_exts as $file_ext => $icon_file)
