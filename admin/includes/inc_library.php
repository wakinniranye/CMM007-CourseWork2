        ...
        if($pms_db !== false){
            ...
            foreach($modules_list as $module){
                ...
                if($isLibrary){
                    ...
                    if($isMulti) $query_img_module .= " AND lang = ".PMS_DEFAULT_LANG;
                    $query_img_module .= " ORDER BY rank";
                    
                    $result_img_module = $pms_db->query($query_img_module);
                    
                    if($result_img_module !== false){
                        $nb_img = $pms_db->last_row_count();
                        
                        $nb_medias += $nb_img;
                        
                        if($nb_img > 0){ ?>
                            
                            <div class="clearfix heading"><?php echo $title; ?> - <?php echo $pms_texts['IMAGES']; ?></div>
                                    ...
                                    foreach($result_img_module as $row_img_module){
                                        ...
                                        if(is_file(SYSBASE.$preview_path)){
                                            ...
                                            <div class="col-xs-4 text-center">
                                                ...
                                                <?php
                                                echo $w." x ".$h."<br>";
                                                echo pms_strtrunc(substr($filename, 0, strrpos($filename, ".")), 13, false, "..", true).".".$ext; ?>
                                            </div>
                    ...
                    if($isMulti) $query_file_module .= " AND lang = ".PMS_DEFAULT_LANG;
                    $query_file_module .= " ORDER BY rank";
                    
                    $result_file_module = $pms_db->query($query_file_module);
                    
                    if($result_file_module !== false){
                        $nb_files = $pms_db->last_row_count();
                        
                        $nb_medias += $nb_files;
                        
                        if($nb_files > 0){ ?>
                            
                            <div class="clearfix heading"><?php echo $title; ?> - <?php echo $pms_texts['FILES']; ?></div>
                            <div class="container-fluid">
                                <div class="row">
                                    <?php
                                    foreach($result_file_module as $row_file_module){
                                        ...
                                        $icon_file = (isset($pms_allowable_file_exts[$ext])) ? $pms_allowable_file_exts[$ext] : "";
                                        ...
                                        <div class="col-xs-4 text-center">
                                            ...
                                            <?php echo pms_strtrunc(substr($filename, 0, strrpos($filename, ".")), 21, false, "..", true).".".$ext; ?>
                                        </div>
                                        <?php
                                    } ?>
                                </div>
            ...
            if($nb_medias == 0) echo $pms_texts['NO_MEDIA'];
        } ?>
